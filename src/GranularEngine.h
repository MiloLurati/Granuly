...
struct Grain 
{
    ...
    float offsetSample = 0.0f; // the current offset we are at while the grain is playing
    bool isActive = false;
    int durationInSamples = 0;
    float grainGain = 0;
    int bufferStartIndex = 0;

    void spawn(int bufferStartingIndex, int startingSample, int duration, float gain)
    {
    ...
    {
        if (durationInSamples <= 0) return 0.0f;

        // Calculate a normalized position between 0.0 (start) and 1.0 (end)
        float normalizedProgress = offsetSample / static_cast<float>(durationInSamples);

        // Safety check: if we somehow overshot, clip the envelope to silence
        if (normalizedProgress >= 1.0f) return 0.0f;
        ...
        return 0.5f * (1.0f - std::cos(2.0f * juce::MathConstants<float>::pi * normalizedProgress));
    }
};
...
{
    public:
        GranularEngine(juce::AudioBuffer<float>& audioReservoir, int audioSize, int grainSampleDur, float grainG, int nGrains = 50, int spawnInt = 882);
        ~GranularEngine();
